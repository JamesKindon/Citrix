<#
.SYNOPSIS
    Migrates dedicated Citrix Single Session Desktops to Citrix Cloud DaaS.
.DESCRIPTION
    Migrates dedicated Citrix Single Session Desktops to Citrix Cloud DaaS. 
...
.PARAMETER LogPath
    Optional. Logpath output for all operations. The default is C:\Logs\MigrateDedicatedMachinesDaaS.log
...
.PARAMETER TargetDeliveryGroup
     Optional. The Delivery Group where machines will go. If not specified, the target cloud instance will be queried and a list of appropriate Delivery Groups will be presented.
.PARAMETER TargetHostingConnection
    Optional. The name of the hosting connection hosting the machine in the target Cloud site. If not specified, the target Cloud Site will be queried and a list of appropriate Hosting Connections will be presented.
...
.PARAMETER SetMaintenanceModeInTarget
    Optional. Allows setting maintenance mode on the machines moved to the target Cloud instance.
...
.PARAMETER MaxRecordCount
    Optional. The max number of machines to be queried in the source site. The default is 1000.
.PARAMETER Whatif
...
if (!$TargetCatalog) {
    ...
    #----------------------------------------------------------------------------------------------------------------------------
    $Method = "Get"
    $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/MachineCatalogs/"
    #----------------------------------------------------------------------------------------------------------------------------
    try {
        $daas_catalogs = (Invoke-RestMethod -Method $Method -Headers $headers -Uri $RequestUri -ErrorAction Stop).Items
    }
    catch {
        Write-Log -Message "[Script Params] Failed to retrieve a list of available Catalogs from Citrix Cloud" -Level Warn
        Write-Log -Message $_ -Level Warn
        StopIteration
        Exit 1
    }

    $TargetCatalog = ($daas_catalogs | Where-Object {$_.ProvisioningType -eq "Manual" -and $_.SessionSupport -eq "SingleSession" -and $_.IsPowerManaged -eq "True"} | Select-Object Name, FullName, AllocationType, ProvisioningType, SessionSupport, ZoneName, Uid, Id | Out-GridView -PassThru -Title "Select a Destination Catalog").Name
...
if (!$TargetDeliveryGroup) {
    ...
    #----------------------------------------------------------------------------------------------------------------------------
    try {
        $daas_delivery_groups = (Invoke-RestMethod -Method $Method -Headers $headers -Uri $RequestUri -ErrorAction Stop).Items
    }
    catch {
        Write-Log -Message "[Script Params] Failed to retrieve a list of available Delivery Groups from Citrix Cloud" -Level Warn
        Write-Log -Message $_ -Level Warn
        StopIteration
        Exit 1
    }
    $TargetDeliveryGroup = ($daas_delivery_groups | Where-Object {$_.SessionSupport -eq "SingleSession" -and $_.DeliveryType -eq "DesktopsOnly" } | Select-Object Name, DeliveryType, Description, Enabled, SessionSupport, Uid, Id | Out-GridView -PassThru -Title "Select a Desktop Group").Name
...
if (!$TargetHostingConnection) {
    ...
    #----------------------------------------------------------------------------------------------------------------------------
    $Method = "Get"
    $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/hypervisors/"
    #----------------------------------------------------------------------------------------------------------------------------
    try {
        $hosting_connections = (Invoke-RestMethod -Method $Method -Headers $headers -Uri $RequestUri -ErrorAction Stop).Items

    }
    catch {
        Write-Log -Message "[Script Params] Failed to retrieve a list of available Hosting Connections from Citrix Cloud" -Level Warn
        Write-Log -Message $_ -Level Warn
        StopIteration
        Exit 1
    }

    $TargetHostingConnection = ($hosting_connections | Select-Object Name, ConnectionType, PluginId, InMaintenanceMode, Uid, Id | Out-GridView -PassThru -Title "Select a Hosting Connection").Name
...
try {
    ...
    Write-Log -Message "[Citrix Cloud] Failed to retrieve a list of available Catalogs from Citrix Cloud" -Level Warn
    Write-Log -Message $_ -Level Warn
    StopIteration
    Exit 1
}

$target_catalog_details = $daas_catalogs | Where-Object {$_.Name -eq $TargetCatalog}

if (!$target_catalog_details) {
    Write-Log -Message "[Citrix Catalog $($TargetCatalog)] Failed to retreive Catalog" -Level Warn
    StopIteration
    Exit 1
}
...
try {
    ...
    Write-Log -Message "[Citrix Cloud] Failed to retrieve a list of available Delivery Groups from Citrix Cloud" -Level Warn
    Write-Log -Message $_ -Level Warn
    StopIteration
    Exit 1
}

$target_delivery_group_details = $daas_delivery_groups | Where-Object {$_.Name -eq $TargetDeliveryGroup}

if (!$target_delivery_group_details) {
    Write-Log -Message "[Citrix Delivery Group $($TargetDeliveryGroup)] Failed to retreive Delivery Group" -Level Warn
    StopIteration
    Exit 1
}

# Validate Single Session Delivery Group
...
# Validate Provisioning Catalogs
if ($target_delivery_group_details.TotalDesktops -eq 0) {
    ...
    Write-Log -Message "[Citrix Delivery Group $($TargetDeliveryGroup)] Has $($target_delivery_group_details.TotalDesktops) members. Identifying if this is a supported Delivery Group based on Provisioning types of asssociated Catalogs" -Level Info
    # Figure out the Catalogs on the Delivery Groups
    #----------------------------------------------------------------------------------------------------------------------------
    # Set API call detail
    #----------------------------------------------------------------------------------------------------------------------------
    $Method = "Get"
    $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/DeliveryGroups/$($target_delivery_group_details.Id)/MachineCatalogs"
    #----------------------------------------------------------------------------------------------------------------------------
    try {
        $target_delivery_group_catalogs = (Invoke-RestMethod -Method $Method -Headers $headers -Uri $RequestUri -ErrorAction Stop).Items
    }
    catch {
        Write-Log -Message "[Citrix Delivery Group $($TargetDeliveryGroup)] Unable to validate source Catalog details for Delivery Group" -Level Warn
        StopIteration
        Exit 1
    }

    if ($target_delivery_group_catalogs.ProvisioningType -contains "MCS" -or $target_delivery_group_catalogs.ProvisioningType -contains "PVS") {
        Write-Log -Message "[Citrix Delivery Group $($TargetDeliveryGroup)] Contains $($target_delivery_group_catalogs.ProvisioningType) Provisioned machines and cannot be used" -Level Warn
        StopIteration
        Exit 1
    }
    else {
...
#----------------------------------------------------------------------------------------------------------------------------
$Method = "Get"
$RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/hypervisors/$($TargetHostingConnection)"
#----------------------------------------------------------------------------------------------------------------------------
try {
    $target_hosting_connection_detail = Invoke-RestMethod -Method $Method -Headers $headers -Uri $RequestUri -ErrorAction Stop
}
catch {
...
if (!$target_hosting_connection_detail) {
    Write-Log -Message "[Citrix Hosting] Citrix Hosting Connection was not found in Citrix Cloud" -Level Warn
    StopIteration
    Exit 1
}
else {
    Write-Log -Message "[Citrix Hosting] Hypervisor Plugin type is: $($target_hosting_connection_detail.PluginId)" -Level Info
}
...
if ($TargetMachinescope -eq "Catalog") {
    if ($source_catalog_hypervisor_connection_type -ne $target_hosting_connection_detail.PluginId) {
        Write-Log -Message "[Hosting Validation Error] Source Hosting Connection is: $($source_catalog_hypervisor_connection_type) but Target Hosting Connection is: $($target_hosting_connection_detail.PluginId). You cannot move a VM between different hosting connection types with this script" -Level Warn
        StopIteration
        Exit 1
    }
}
...
foreach ($vm in $target_vms) {
    ...
    try {
            ...
            if ($IncludeMCSMachinesFromSource) {
                ...
                if ($catalog_supported_clone_type_check_completed -eq $false) {
                    ...
                    try {
                        ...
                        if ($vm_hypervisor_connection_type -ne $target_hosting_connection_detail.PluginId) {
                            Write-Log -Message "[VM $($vm)] Hosting Validation Error. Source Hosting Connection is: $($vm_hypervisor_connection_type) but Target Hosting Connection is: $($target_hosting_connection_detail.PluginId). You cannot move a VM between different hosting connection types with this script" -Level Warn
                            $failed_vm_count ++
                            Continue
                        }
                        else {
                            Write-Log -Message "[VM $($vm)] Hosting connection types match. Source Hosting Connection is: $($vm_hypervisor_connection_type) and Target Hosting Connection is: $($target_hosting_connection_detail.PluginId)." -Level Info
                        }
        ...
        #----------------------------------------------------------------------------------------------------------------------------
        $Method = "Post"
        $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/Machines/`$Search"
        $PayloadContent = @{
            SearchFilters = @(
                @{
                    Property = "MachineName"
                    Value = $source_vm_details.MachineName
                    Operator = "Equals"
                }
            )
        }
        ...
        try {
            $machine_exists_in_target = Invoke-RestMethod -Method $Method -Headers $headers -Body $Payload -Uri $RequestUri -ContentType "application/json" -ErrorAction Stop
        }
        catch {
            Write-Log -Message "[VM $($vm)] VM does not exist in Target Site" -Level Info
        }

        if ($machine_exists_in_target.TotalItems -gt 0){
        ...
        if (!$Whatif) {
            ...
            #----------------------------------------------------------------------------------------------------------------------------
            $Method = "Post"
            $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/MachineCatalogs/$($target_catalog_details.id)/Machines"
            $PayloadContent = @{
                MachineName = $source_vm_details.MachineName
                HostedMachineId = $source_vm_details.HostedMachineId
                HypervisorConnection = $target_hosting_connection_detail.Name
            }
            $Payload = ($PayloadContent | ConvertTo-Json)
            #----------------------------------------------------------------------------------------------------------------------------
            try {
                $target_machine_to_catalog = Invoke-RestMethod -Method $Method -Headers $headers -Body $Payload -Uri $RequestUri -ContentType "application/json" -ErrorAction Stop
                if ($target_machine_to_catalog.status -eq "Failed") {
                    Write-Log -Message "[VM $($vm)] Failed to Add machine to Catalog with error: $($target_machine_to_catalog.ErrorString)" -Level Warn
                    $failed_vm_count ++
                    Continue
                }
                elseif ($target_machine_to_catalog.status -eq "Complete") {
                    Write-Log -Message "[VM $($vm)] Added to target site Catalog" -Level Info
                }
            }
            catch {
                $failed_vm_count ++
            ...
            Write-Log -Message "[WHATIF] [VM $($vm)] Would add to Citrix Cloud Site" -Level Info
            ...
            Write-Log -Message "[WHATIF] [VM $($vm)] Hypervisor Conection name: $($target_hosting_connection_detail.Name)" -Level Info
            Write-Log -Message "[WHATIF] [VM $($vm)] Hypervisor Connection Uid: $($target_hosting_connection_detail.Uid)" -Level Info
        }
        #endregion Add Machine to Catalog

        #region Get New Machine Details
        #----------------------------------------------------------------------------------------------------------------------------
        # Set API call detail
        #----------------------------------------------------------------------------------------------------------------------------
        $Method = "Post"
        ...
        #----------------------------------------------------------------------------------------------------------------------------
        try {
            $target_machine_detail = (Invoke-RestMethod -Method $Method -Headers $headers -Body $Payload -Uri $RequestUri -ContentType "application/json" -ErrorAction Stop).Items
            if ($null -ne $target_machine_detail) {
                Write-Log -Message "[VM $($vm)] Details retrieved from Cloud Site" -Level Info
            }
            else {
                Write-Log -Message "[VM $($vm)] Could not find machine details in Cloud Site" -Level Warn
                $failed_vm_count ++
                Continue
            }
        }
        ...
        #endregion Get New Machine Details
        ...
        if ($SetMaintenanceModeInTarget) {
            if (!$Whatif) {
                ...
                #----------------------------------------------------------------------------------------------------------------------------
                $Method = "Patch"
                $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/Machines/$($target_machine_detail.Id)"
                $PayloadContent = @{
                ...
                try {
                    ...
                    $target_machine_in_maintenance = Invoke-RestMethod -Method $Method -Headers $headers -Body $Payload -Uri $RequestUri -ContentType "application/json" -ErrorAction Stop
                }
                catch {
        ...
        if (!$Whatif) {
            ...
            $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/Machines/$($target_machine_detail.Id)"
            $PayloadContent = @{
                PublishedName = $machine_published_name
            }
            $Payload = ($PayloadContent | ConvertTo-Json)
            #----------------------------------------------------------------------------------------------------------------------------
            try {
                Write-Log -Message "[VM $($vm)] Adding VM to delivery group $($target_delivery_group_details.Name)" -Level Info
                $target_machine_to_delivery_group = Invoke-RestMethod -Method $Method -Headers $headers -Body $Payload -Uri $RequestUri -ContentType "application/json" -ErrorAction Stop
            }
            catch {
        ...
        if (!$Whatif) {
            ...
            if ($null -eq $machine_published_name) {
                ...
                #----------------------------------------------------------------------------------------------------------------------------
                $Method = "Post"
                $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/DeliveryGroups/$($target_delivery_group_details.Id)/Machines"
                $PayloadContent = @{
                    PublishedName = $machine_published_name
                }
                $Payload = ($PayloadContent | ConvertTo-Json)
                ...
                try {
                    ...
                    $machine_published_name_set = Invoke-RestMethod -Method $Method -Headers $headers -Body $Payload -Uri $RequestUri -ContentType "application/json" -ErrorAction Stop
                }
                catch {
        ...
        if ($AssignedUsers) {
            if (!$Whatif) {
                ...
                try {
                    $vm_assignments = Invoke-RestMethod -Method $Method -Headers $headers -Body $Payload -Uri $RequestUri -ErrorAction Stop -ContentType "application/json"
                }
                catch {
                    Write-Log -Message "[VM $($vm)] Failed to assign users" -Level Warn
                    $failed_vm_count ++
                }
            }
            else {
...
if ($ResetTargetHostingConnection) {
    if ($successful_vm_count -gt 0) {
        if (!$Whatif) {
            ...
            #----------------------------------------------------------------------------------------------------------------------------
            $Method = "POST"
            $RequestUri = "https://$($CloudUrl)/cvadapis/$SiteID/hypervisors/$($target_hosting_connection_detail.Name)/`$resetConnection"
            $Payload = $null
            #----------------------------------------------------------------------------------------------------------------------------
            try {
                $update_hosting_connection = Invoke-RestMethod -Method $Method -Headers $headers -Uri $RequestUri -Body $Payload -ContentType "application/json" -ErrorAction Stop
            }
            catch {
